...
import type { ComponentData, FunctionGroup, TargetEnvFile } from '../../types/wadi-types';
...
function downloadComponent(destDir: string, Bucket: string) {
   ...
   */
  return async ([component, componentData]: [string, ComponentData]): Promise<number> => {
...
async function downloadArtifacts(
  ...
  const files = Object.entries(components).filter(
    ([, { build_type }]) => !buildTypeList || buildTypeList.includes(build_type || ''),
  );
