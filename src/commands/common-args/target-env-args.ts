...
import type { ComponentData, ComponentMap, PkgJson } from '../../types/wadi-types';
...
export async function processTargetEnvCliArgs({
  ...
  if (!componentNames || componentNames.length === 0) {
    ...
    componentMap = Object.fromEntries(componentNames.split(',').map((comp) => [comp, {}]));
  }

  if (!versionRoot) {
    const packageDotJson = await readJsonFile<PkgJson>(`${projRootDir}/package.json`);
