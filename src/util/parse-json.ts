...
export async function readTargetEnvJson(targetEnvJsonPath: string): Promise<TargetEnvFile> {
  const targetEnvJson = parseJson<TargetEnvFile>(await readFile(targetEnvJsonPath));

  if (!targetEnvJson.components) {
    throw new Error(`"components" element not found in json, see '${targetEnvJsonPath}'`);
  }
