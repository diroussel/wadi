...
function isReadable(body: Readable | ReadableStream | Blob | undefined): body is Readable {
 ...
 */
async function writeToFile(inputStream: Readable, filePath: string): Promise<number> {
...
export async function getS3Object(location: S3Location, defaultValue?: string): Promise<string> {
  try {
...
export async function writeS3ObjectToFile(location: S3Location, filename: string): Promise<number> {
  try {
...
export async function listS3Objects({ Bucket, Key }: S3Location): Promise<string[]> {
  try {
    const parameters = {
      Bucket,
      Key,
