...
describe('getComponents', () => {
  it('an error is thrown if no workspaces can be read from root package.json', async () => {
    ...
    await expect(async () => getComponents('rootDir', '')).rejects.toThrowError(
  ...
  it('glob should be called with workspaces, and projRootDir arg', async () => {
    ...
    expect(glob).toBeCalledWith('testRootDirectory/{workspaceDir1,workspaceDir2}/package.json', {
      ignore: '**/node_modules/**',
    });
  });
  ...
  it('readJsonFile will be called with each path returned by glob', async () => {
    readJsonFileMock.mockImplementation(async (path): Promise<PkgJson> => {
      switch (path) {
        ...
        case 'and/second/path/package.json': {
          return {
            version: '1.0.0',
    ...
    globMock.mockResolvedValueOnce(['first/path/package.json', 'and/second/path/package.json']);
